...
/* const sendEmail = require('../config/mailTransporter.js'); */
/* import sendMail from '../config/mailTransporter.js' */
    ...
    if(!found.length){
        ...
        transporter.sendMail(mailOptions, function(error, info){
            if (error) {
             ...
             res.status(201).send({ message:"Error sending activation email"}); 
            } else {
...
/* export const handleRegisterUser = async (req, res) => {
    try {
        const { new_User } = req.body;
        if (!new_User) {
            return res.status(400).json({ message: "User Details not received" });
        }
        ...
        if (!found.length) {
            const salt = await bcrypt.genSalt(10);
            const hashedPassword = await bcrypt.hash(new_User.password, salt);
            const hashedUser = { ...new_User, password: hashedPassword, isActivated: false };
            let addUser = new User({ ...hashedUser });
            const result = await addUser.save();
...
/* export const handleRegisterUser = async (req, res) => {
    try {
        ...
        if (!found.length) {
            ...
            const activationLink = `${client_URL}/activate/${result._id}?activateToken=${validateToken}`;
            await sendEmail(result.email, 'Account Activation Link', `Click the following link to activate your account: ${activationLink}`);
...
export const handleLogin = async (req, res) => {
    try {
        ...
        if (!foundUser.isActivated) return res.status(401).json({ message: "User account not activated" });

        const validPwd = await bcrypt.compare(password, foundUser.password);


        const USER_ACCESS_TOKEN_SECRET = 'CRESCENTINFOTECH';
        if (validPwd) {
            const accessToken = jwt.sign(
                {
                    userDetail: {
